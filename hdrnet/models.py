...
class HDRNetCurves(object):
    ...
    @classmethod
    def inference(cls, lowres_input, fullres_input, lr_params,
                  config, is_training=False):

        batch_size, w, h, channels = lowres_input.get_shape().as_list()

        with tf.variable_scope('coefficients'):
            expanded_lr_params = tf.reshape(lr_params, [batch_size, 1, 1, -1])
            expanded_lr_params = tf.tile(expanded_lr_params, [1, w, h, 1])

            lowres_and_params = tf.concat([lowres_input, expanded_lr_params], 3)
            bilateral_coeffs = cls._coefficients(lowres_and_params, config, is_training)
            tf.add_to_collection('bilateral_coefficients', bilateral_coeffs)

        with tf.variable_scope('guide'):
            guide = cls._guide(fullres_input, config, is_training)
    ...
    @classmethod
    def _coefficients(cls, input_tensor, config, is_training):
        batch_size = input_tensor.get_shape().as_list()[0]
        grid_depth = config['luma_bins']
        channel_multiplier = config['channel_multiplier']
        spatial_bin = config['spatial_bin']

        # -----------------------------------------------------------------------
        with tf.variable_scope('splat'):
            n_ds_layers = int(np.log2(config['net_input_size'] / spatial_bin))
        ...
        with tf.variable_scope('global'):
            ...
            for i in range(n_global_layers):
                current_layer = conv(current_layer, 8 * channel_multiplier * grid_depth, 3, stride=2,
                                     batch_norm=config['batch_norm'], is_training=is_training,
                                     scope="conv{}".format(i + 1))
            _, lh, lw, lc = current_layer.get_shape().as_list()
            current_layer = tf.reshape(current_layer, [batch_size, lh * lw * lc])

            current_layer = fc(current_layer, 32 * channel_multiplier * grid_depth,
                               batch_norm=config['batch_norm'], is_training=is_training,
                               scope="fc1")
            current_layer = fc(current_layer, 16 * channel_multiplier * grid_depth,
                               batch_norm=config['batch_norm'], is_training=is_training,
                               scope="fc2")
            # don't normalize before fusion
            current_layer = fc(current_layer, 8 * channel_multiplier * grid_depth, activation_fn=None, scope="fc3")
        ...
        with tf.variable_scope('local'):
            current_layer = splat_features
            current_layer = conv(current_layer, 8 * channel_multiplier * grid_depth, 3,
                                 batch_norm=config['batch_norm'],
                                 is_training=is_training,
                                 scope='conv1')
            # don't normalize before fusion
            current_layer = conv(current_layer, 8 * channel_multiplier * grid_depth, 3, activation_fn=None,
        ...
        with tf.name_scope('fusion'):
            ...
            fusion_global = tf.reshape(global_features, [batch_size, 1, 1, 8 * channel_multiplier * grid_depth])
        ...
        with tf.variable_scope('prediction'):
            ...
            current_layer = conv(current_layer, grid_depth * cls.n_out() * cls.n_in(), 1,
    ...
    @classmethod
    def _guide(cls, input_tensor, config, is_training):
...
class HDRNetPointwiseNNGuide(HDRNetCurves):
    ...
    @classmethod
    def _guide(cls, input_tensor, config, is_training):
        n_guide_feats = config['guide_complexity']
...
class HDRNetGaussianPyrNN(HDRNetPointwiseNNGuide):
    ...
    @classmethod
    def inference(cls, lowres_input, fullres_input, config,
                  is_training=False):

        with tf.variable_scope('coefficients'):
            bilateral_coeffs = cls._coefficients(lowres_input, config, is_training)
            tf.add_to_collection('bilateral_coefficients', bilateral_coeffs)
        ...
        with tf.variable_scope('guide'):
            guide = cls._guide(multiscale, config, is_training)
            for g in guide:
    ...
    @classmethod
    def _guide(cls, multiscale, config, is_training):
        guide_lvls = []
        for il, lvl in enumerate(multiscale):
            with tf.variable_scope('level_{}'.format(il)):
                guide_lvl = HDRNetPointwiseNNGuide._guide(lvl, config, is_training)
